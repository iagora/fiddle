    ...
    if i >= String.length mask then List.rev acc
      ...
      | '?' -> (
          if i + 1 < String.length mask then
            ...
            | 's' -> aux (i + 2) (Charset "!@#$%^&*()-_=+[]{}|;:',.<>?/~`" :: acc)
            | '?' -> aux (i + 2) (Static '?' :: acc)
            | _ -> failwith "Invalid mask"
          else failwith "Invalid mask"
        )
...
let rec generate_and_print_combinations elements acc =
  match elements with
  | [] -> print_endline acc
  | Static c :: rest -> generate_and_print_combinations rest (acc ^ (String.make 1 c))
  | Charset cs :: rest ->
      String.iter (fun c -> generate_and_print_combinations rest (acc ^ (String.make 1 c))) cs
