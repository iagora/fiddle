let cpf_to_digest f mask cpf_input =
  let open Core in
  ...
  in
  let check_digits = Util.calculate_cpf_digits cpf_base in
  ...
  Some (Printf.sprintf "%s-%d%d%s\t%s%s\t%s\n%!" extracted x y ast cpf ast digest)

let digest_to_cpf f mask (start, finish) target_result =
  let open Core in
  let mappings =
    String.to_list mask
  ...
  let rec search n =
    if n > finish then None (* Limiting the range for simplicity *)
      ...
      let cpf_base = Util.int_to_cpf_array n in
      let check_digits = Util.calculate_cpf_digits cpf_base in
      let cpf_digits = process_mask 0 0 check_digits [] in
